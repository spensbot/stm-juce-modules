...
public:
    ...
    float push(float val){
        ...
        if (val > max) {
            ...
            max = 0.0f;
            maxCount = 0;
            for (int i=0 ; i<regionMaxs.size() ; i++){
                if (regionMaxs[i] > max) {
                    max = regionMaxs[i];
...
public:
    ...
    {
        float sustainSamples = spec.sampleRate * sustain / 1000.0f;
        levelTracker.prepare(sustainSamples);
        gainValues.setSize(spec.numChannels, spec.maximumBlockSize);
        reset();
    }
    ...
    void process(const dsp::ProcessContextReplacing<float>& context)
    {
        ...
        for (auto sampleIndex=0 ; sampleIndex<inBlock.getNumSamples() ; sampleIndex++)
        {
            ...
            for (auto channelIndex=0 ; channelIndex<inBlock.getNumChannels() ; channelIndex++)
            {
    ...
    {
        dsp::AudioBlock inBlock = context.getInputBlock();
        dsp::AudioBlock outBlock = context.getOutputBlock();
