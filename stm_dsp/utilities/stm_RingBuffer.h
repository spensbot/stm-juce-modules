...
public:
    ...
    void prepare(const dsp::ProcessSpec& spec, double pullsPerSecond = 60.0f, int safetyFactor = 4){
        ...
        const int samplesPerPush = spec.maximumBlockSize;
        ...
        buffer.setSize(spec.numChannels, bufferSamples);
    ...
    const juce::dsp::AudioBlock<float> popBlock(){
        ...
        return juce::dsp::AudioBlock<float>(buffer).getSubBlock(startPos, readLength);
    }
